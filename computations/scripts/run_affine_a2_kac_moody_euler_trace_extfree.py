"""Run affine A2 Euler trace in the 5D Kac-Moody ambient realization."""
...
DEFAULT_OUTPUT = Path(
    "computations/euler_trace_outputs/affine_a2_kac_moody_extfree_euler_trace_output.txt"
)
...
def kac_moody_affine_a2_realization(diagram: DynkinDiagram) -> Realization:
    """The affine A2 Kac-Moody ambient realization on ``h^*``.

    The coordinate basis is

        epsilon_1, epsilon_2, epsilon_3, delta, Lambda_0.

    This is the 5D ambient model.  The fixed trace line
    ``epsilon_1 + epsilon_2 + epsilon_3`` may be quotiented to obtain the
    4D minimal Kac-Moody realization.
    """

    roots = {
        0: sp.Matrix([-1, 0, 1, 1, 0]),
        1: sp.Matrix([1, -1, 0, 0, 0]),
        2: sp.Matrix([0, 1, -1, 0, 0]),
    }
    coroot_pairings_on_basis = {
        0: sp.Matrix([-1, 0, 1, 0, 1]),
        1: sp.Matrix([1, -1, 0, 0, 0]),
        2: sp.Matrix([0, 1, -1, 0, 0]),
    }
    ...
    for vertex in diagram.vertices:
        alpha = roots[vertex]
        alpha_coroot = coroot_pairings_on_basis[vertex].T
...
def _run(
    ...
    *,
    ...
    logger.log("Affine A2 Kac-Moody 5D Euler-trace computation (Ext-free)")
    ...
    realization = kac_moody_affine_a2_realization(diagram)
    logger.timing("setup Kac-Moody ambient realization", section_start)
    logger.log(f"basis: epsilon_1, epsilon_2, epsilon_3, delta, Lambda_0")
    ...
    for choices, model in sorted(
        ...
        generator_degrees = extfree_generator_q_degrees_by_degree(
            ...
            validate=validate,
            context=f"Kac-Moody affine A2 Ext-free term {choices}",
        )
    ...
    logger.log("")
    logger.log("Final Kac-Moody Ext-free Euler trace:")
