"""Run the type A3 Euler-trace computation with timing logs.

This script computes the Euler trace for the braid

    (1,+), (2,+), (3,+), (1,+), (2,+), (3,+)

in the finite type A3 Coxeter graph with edges 12 and 23, using the standard
...
DEFAULT_OUTPUT = Path("computations/euler_trace_outputs/a3_euler_trace_output.txt")
...
def main() -> None:
    ...
    if args.caffeinate and not os.environ.get("A3_EULER_TRACE_CAFFEINATED"):
        env = os.environ.copy()
        env["A3_EULER_TRACE_CAFFEINATED"] = "1"
        command = [
            "/usr/bin/caffeinate",
            "-dimsu",
            sys.executable,
            "-m",
            "computations.slower_old_euler_trace.run_a3_euler_trace",
            "--output",
            str(args.output),
        ]
    ...
    with args.output.open("w", encoding="utf-8") as handle:
        ...
        logger.log("Type A3 Euler-trace computation")
        ...
        logger.log("Example:")
        logger.log(
            "  python3 -m computations.slower_old_euler_trace.run_a3_euler_trace --caffeinate "
